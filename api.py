...
class UserModal(db.Model): # Define User model 
    ...
    def __repr__(self): # String representation
        return f"User (name = {self.name}, email = {self.email})" # Define how the User model is represented
...
class Users(Resource): # Define Users resource for API
    ...
    def post(self):
        ...
        user = UserModal(name=args['name'], email=args['email'])
        db.session.add(user)
        db.session.commit()
        users = UserModal.query.all()
        return users, 201
...
class User(Resource): # Define User resource for API
    ...
    def patch(self, id):
        ...
        user.name = args['name'] # Update name and email
